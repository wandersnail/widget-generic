...
class HeaderGridView : GridView {
    ...
    private inner class FullWidthFixedViewLayout(context: Context) : FrameLayout(context) {
        override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
            ...
            val targetWidth = (this@HeaderGridView.measuredWidth - this@HeaderGridView.paddingLeft - this@HeaderGridView.paddingRight)
            wMeasureSpec = View.MeasureSpec.makeMeasureSpec(targetWidth, View.MeasureSpec.getMode(wMeasureSpec))
    ...
    private class HeaderViewGridAdapter( // This ArrayList is assumed to NOT be null.
        ...
        override fun getView(position: Int, convertView: View?, parent: ViewGroup): View? {
            ...
            val numHeadersAndPlaceholders = headersCount * mNumColumns
            if (position < numHeadersAndPlaceholders) {
                val headerViewContainer = mHeaderViewInfos[position / mNumColumns].viewContainer
                return if (position % mNumColumns == 0) {
                    headerViewContainer
                } else {
                    if (view == null) {
                    ...
                    view.minimumHeight = headerViewContainer!!.height
                    view
                }
            }
            // Adapter
            ...
            if (mAdapter != null) {
                adapterCount = mAdapter.count
                if (adjPosition < adapterCount) {
                    return mAdapter.getView(adjPosition, view, parent)
                }
            }
            throw ArrayIndexOutOfBoundsException(position)
        }
